 ...
 */
void sr_arpcache_sweepreqs(struct sr_instance *sr) { 
    /*handle_arpreq(struct sr_instance* sr, struct sr_arpreq *cache_req_ptr)*/
    struct sr_arpreq *cache_reqs = (sr->cache).requests;
    struct sr_arpreq *reqs_ptr = cache_reqs;
    struct sr_arpreq *next_ptr;
    
    if (reqs_ptr) {
        next_ptr = reqs_ptr->next;
    }
    else {
        next_ptr = NULL;
    }

    /*Go through each request and call handle_arpreq on it*/
    while (reqs_ptr) {
        handle_arpreq(sr, reqs_ptr);
        reqs_ptr = next_ptr;
